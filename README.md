...
The current app sctructure is working off of 5 pods, 2 PVCs, and 3 services. This structure has been replicated in both a test and prod environment. The app currenlyt built in the prod environment is being pushed to the web by a NodePoint service from ISP so that users can interact with the software via a more friendly interface.
...
service/avocado-test-flask-service unchanged
deployment.apps/avocado-test-redis-pvc-deployment unchanged
persistentvolumeclaim/avocado-test-redis-pvc unchanged
service/avocado-test-redis-service unchanged
deployment.apps/avocado-test-worker-deployment unchanged
...
The postgres deployment, service, and pvc are very similar to the redis structure, but its addition was necessary in order to keep the storage of the job requests and the actual avocado data seperate.
